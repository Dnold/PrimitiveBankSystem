...
public static class BankInterface
{
    ...
    private static void PrintTransferMoney(Account sender)
    {
        ...
        BankSystem.PerformTransaction(sender.IBAN, recipientIBAN, amount);
            ...
            case "1":
                ...
                if (BankSystem.IsBICTaken(chosenBIC))
                {
                    ...
                    if (option2 == "1")
                    {
                        ...
                        Account newAcc = choosenBank.CreateAccount(chosenName, chosenPassword);
                        Console.Clear(); Console.WriteLine("Your Account has been created");
    ...
    {
        Console.Clear();
        Console.WriteLine("Create New Bank");
        Console.WriteLine("Enter your prefered action");
...
public static class BankSystem
{
    ...
    public static Bank CreateBank(byte bic)
    {
        ...
        banks.Add(bank);
        return banks.First(e => e.BIC == bank.BIC);
    }
    public static Bank GetBankByBIC(byte bic)
    {
        return banks.FirstOrDefault(e => e.BIC == bic);
    }
    public static bool IsBICTaken(byte BIC)
    {
        if (banks.FirstOrDefault(e => e.BIC == BIC) != null)
        {
            return true;
        }
        return false;
    ...
    public static void PerformTransaction(int senderIBAN, int receiverIBAN, double amount)
    {
        Bank? senderBank = banks.FirstOrDefault(e => e.BIC == BankUtils.GetBICFromIBAN(senderIBAN));
        Bank? receiverBank = banks.FirstOrDefault(e => e.BIC == BankUtils.GetBICFromIBAN(receiverIBAN));
...
public class Bank
{
    ...
    public Account CreateAccount(string ownerName, string password)
    {
        ...
        return accounts.First(e => e.id == newAccount.id);
    }
